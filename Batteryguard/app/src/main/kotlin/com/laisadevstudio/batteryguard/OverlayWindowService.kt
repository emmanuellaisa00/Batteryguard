...
class OverlayWindowService : Service() {
    ...
    private fun buildNotification(): Notification {
        val reasons = BatteryGuardService.activeReasons
        val text = when {
            reasons.isEmpty() -> "BatteryGuard lock overlay active"
            GuardReason.LOW_BATTERY in reasons ->
                "Locked — charge to ${GuardRules.batteryUnlockTarget(this)}%+ or clear other active rules"
            else -> GuardRules.reasonsCsv(reasons)
        }
